...
for base in range(base_max):
    ...
    for col in range(base, pc.size_mat - base - 1):
        string_mat[row_ind][base], string_mat[base][col] = (
            string_mat[base][col],
            string_mat[row_ind][base],
        )
        row_ind -= 1

    col_ind = base
    for row in range(base, pc.size_mat - base - 1):
        string_mat[base][col_ind], string_mat[row][pc.size_mat - base - 1] = (
            string_mat[row][pc.size_mat - base - 1],
            string_mat[base][col_ind],
        )
        col_ind += 1
    ...
    for col in range(pc.size_mat - base - 1, base, -1):
        (
            string_mat[row_ind][pc.size_mat - base - 1],
            string_mat[pc.size_mat - base - 1][col],
        ) = (
            string_mat[pc.size_mat - base - 1][col],
            string_mat[row_ind][pc.size_mat - base - 1],
        )
